...
<div class="container-fluid mt-n4 pb-5 px-2 px-md-4" style="position: relative; z-index: 20;">
    <style>
        ...
        @media (max-width: 768px) { .minimal-toolbar { padding: 5px 10px; gap: 5px; flex-wrap: wrap; border-radius: 12px;} .btn-filter-trigger span { display: none; } .btn-filter-trigger { padding: 10px; flex-grow:1; justify-content:center;} .search-input-group { width: 100%; } }
    </style>

    <div class="minimal-toolbar">
        <form id="formFiltrosReportes" method="GET" class="d-flex flex-grow-1 align-items-center gap-2 m-0 w-100 w-md-auto">
            <div class="search-input-group">
                <i class="bi bi-search"></i>
                <input type="text" name="buscar" placeholder="Buscar ticket o cliente..." value="<?php echo htmlspecialchars($buscar); ?>" autocomplete="off">
            </div>

            <input type="hidden" name="f_inicio" id="hiddenDesde" value="<?php echo htmlspecialchars($inicio); ?>">
            <input type="hidden" name="f_fin" id="hiddenHasta" value="<?php echo htmlspecialchars($fin); ?>">
            <input type="hidden" name="id_usuario" id="hiddenUsuario" value="<?php echo htmlspecialchars($id_usuario); ?>">
            <input type="hidden" name="metodo" id="hiddenMetodo" value="<?php echo htmlspecialchars($metodo); ?>">

            <button type="button" class="btn btn-light border btn-filter-trigger shadow-sm" onclick="abrirFiltrosReportes()">
                <i class="bi bi-sliders2 text-primary"></i> <span>FILTROS</span>
            </button>
            <button type="submit" class="btn btn-primary btn-filter-trigger shadow-sm me-md-2" style="padding: 8px 15px;">
                <i class="bi bi-arrow-right-short" style="font-size: 1.3rem;"></i>
            </button>
        </form>

        <div class="d-flex gap-2">
            <a href="?set_rango=hoy" class="btn btn-warning text-dark btn-filter-trigger shadow-sm">
                <i class="bi bi-calendar-event-fill"></i> <span>HOY</span>
            </a>
            <a href="?set_rango=mes" class="btn btn-info text-white btn-filter-trigger shadow-sm">
                <i class="bi bi-calendar3"></i> <span>MES</span>
            </a>
        </div>
    </div>

    <script>
    function abrirFiltrosReportes() {
        let usOptions = `<option value="">Todos</option>`;
        ...
        let met = document.getElementById('hiddenMetodo').value;

        Swal.fire({
            title: 'ANÁLISIS AVANZADO',
            html: `
                <div style="text-align: left; display: flex; flex-direction: column; gap: 15px; margin-top: 10px;">
                    <div><label class="small fw-bold text-muted text-uppercase">Fecha Inicio</label><input type="date" id="swal_desde" class="form-control fw-bold" value="${document.getElementById('hiddenDesde').value}"></div>
                    <div><label class="small fw-bold text-muted text-uppercase">Fecha Fin</label><input type="date" id="swal_hasta" class="form-control fw-bold" value="${document.getElementById('hiddenHasta').value}"></div>
                    <div><label class="small fw-bold text-muted text-uppercase">Vendedor</label><select id="swal_us" class="form-select fw-bold">${usOptions}</select></div>
                    <div>
                        <label class="small fw-bold text-muted text-uppercase">Medio de Pago</label>
                        <select id="swal_met" class="form-select fw-bold">
                            <option value="">Todos los medios</option>
                            <option value="Efectivo" ${met === 'Efectivo' ? 'selected' : ''}>💵 Efectivo</option>
                            <option value="mercadopago" ${met === 'mercadopago' ? 'selected' : ''}>📱 MercadoPago QR</option>
                            <option value="Transferencia" ${met === 'Transferencia' ? 'selected' : ''}>🏦 Transferencia</option>
                            <option value="Debito" ${met === 'Debito' ? 'selected' : ''}>💳 Tarjeta de Débito</option>
                            <option value="Credito" ${met === 'Credito' ? 'selected' : ''}>💳 Tarjeta de Crédito</option>
                        </select>
                    </div>
                </div>
            `,
            showCancelButton: true, confirmButtonText: 'GENERAR REPORTE', cancelButtonText: 'LIMPIAR', confirmButtonColor: '#3b82f6', cancelButtonColor: '#ef4444',
            ...
            if (result.isConfirmed) {
                ...
                document.getElementById('hiddenMetodo').value = document.getElementById('swal_met').value;
                document.getElementById('formFiltrosReportes').submit();
            } else if (result.dismiss === Swal.DismissReason.cancel) { window.location.href = 'reportes.php'; }
        });
    }
    </script>
