...
<div class="container-fluid container-md pb-5 mt-n4 px-2 px-md-3" style="position: relative; z-index: 20;">
    ...
    <div class="minimal-toolbar">
        <form id="formFiltrosMermas" method="GET" class="d-flex w-100 align-items-center gap-2 m-0">
            <div class="search-input-group">
                <i class="bi bi-search"></i>
                <input type="text" name="buscar" placeholder="Buscar producto o motivo..." value="<?php echo htmlspecialchars($buscar); ?>" autocomplete="off">
            </div>
            ...
            <button type="button" class="btn btn-light border btn-filter-trigger shadow-sm" onclick="abrirFiltrosMermas()">
    ...
    <script>
    function abrirFiltrosMermas() {
        let usOptions = `<option value="">Todos</option>`;
        <?php 
        $usListM = $conexion->query("SELECT id, usuario FROM usuarios ORDER BY usuario ASC")->fetchAll(PDO::FETCH_ASSOC);
        foreach($usListM as $u): ?> 
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                document.getElementById('formFiltrosMermas').submit();
            } else if (result.dismiss === Swal.DismissReason.cancel) { window.location.href = 'mermas.php'; }
        });
    }
    </script>
        <div class="row g-4">
