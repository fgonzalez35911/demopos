...
<div class="container mt-n4 pb-5" style="position: relative; z-index: 20;">
    ...
    <div class="minimal-toolbar">
        <form id="formFiltrosCajas" method="GET" class="d-flex w-100 align-items-center gap-2 m-0">
            <div class="search-input-group">
                <i class="bi bi-search"></i>
                <input type="text" name="buscar" placeholder="Buscar Nº de Caja o Nombre..." value="<?php echo htmlspecialchars($buscar); ?>" autocomplete="off">
            </div>
            ...
            <input type="hidden" name="id_cliente" id="hiddenCliente" value="<?php echo htmlspecialchars($f_cliente); ?>">
            <input type="hidden" name="id_usuario" id="hiddenUsuario" value="<?php echo htmlspecialchars($f_usuario); ?>">

            <button type="button" class="btn btn-light border btn-filter-trigger shadow-sm" onclick="abrirFiltrosCajas()">
    ...
    <script>
    function abrirFiltrosCajas() {
        let usOptions = `<option value="">Todos los operadores</option>`;
        <?php foreach($usuarios_lista as $u): ?> usOptions += `<option value="<?php echo $u['id']; ?>" ${document.getElementById('hiddenUsuario').value == '<?php echo $u['id']; ?>' ? 'selected' : ''}><?php echo addslashes(strtoupper($u['usuario'])); ?></option>`; <?php endforeach; ?>
        
        let cliOptions = `<option value="">Todos los clientes</option>`;
        <?php foreach($clientes_lista as $c): ?> cliOptions += `<option value="<?php echo $c['id']; ?>" ${document.getElementById('hiddenCliente').value == '<?php echo $c['id']; ?>' ? 'selected' : ''}><?php echo addslashes(strtoupper($c['nombre'])); ?></option>`; <?php endforeach; ?>
        ...
        Swal.fire({
            ...
            html: `
                <div style="text-align: left; display: flex; flex-direction: column; gap: 15px; margin-top: 10px;">
                    ...
                    <div><label class="small fw-bold text-muted text-uppercase">Responsable</label><select id="swal_us" class="form-select fw-bold">${usOptions}</select></div>
                    <div><label class="small fw-bold text-muted text-uppercase">Filtrar por Cliente</label><select id="swal_cli" class="form-select fw-bold">${cliOptions}</select></div>
                </div>
            `,
            ...
            if (result.isConfirmed) {
                ...
                document.getElementById('hiddenCliente').value = document.getElementById('swal_cli').value;
                document.getElementById('formFiltrosCajas').submit();
            } else if (result.dismiss === Swal.DismissReason.cancel) { window.location.href = 'historial_cajas.php'; }
        });
    }
    </script>
