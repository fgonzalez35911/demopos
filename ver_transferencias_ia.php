...
<div class="container-fluid mt-n4 pb-5 px-2 px-md-4" style="position: relative; z-index: 20;">
    ...
    <div class="minimal-toolbar">
        <form id="formFiltrosTransferencias" method="GET" class="d-flex w-100 align-items-center gap-2 m-0">
            <div class="search-input-group">
                <i class="bi bi-search"></i>
                <input type="text" name="buscar" placeholder="Buscar por nombre, DNI, CBU o monto..." value="<?php echo htmlspecialchars($buscar); ?>" autocomplete="off">
            </div>
            ...
            <button type="button" class="btn btn-light border btn-filter-trigger shadow-sm" onclick="abrirFiltrosTransferencias()">
                <i class="bi bi-sliders2 text-primary"></i> <span>FILTROS</span>
            </button>
            
            <a href="?estado=pendiente" class="btn btn-warning btn-filter-trigger shadow-sm text-dark d-none d-md-flex" title="Ver Pendientes">
                <i class="bi bi-clock-history"></i> <span>PENDIENTES</span>
            </a>

            <button type="button" class="btn btn-dark btn-filter-trigger shadow-sm" onclick="abrirModalListaNegra()" title="Lista Negra">
                <i class="bi bi-shield-lock-fill text-danger"></i> <span>L. NEGRA</span>
            </button>

            <button type="button" id="botonMasivo" class="btn btn-danger btn-filter-trigger shadow-sm oculto" onclick="borrarSeleccionados()">
                <i class="bi bi-trash3-fill"></i> <span id="cuenta">0</span>
            </button>

            <button type="submit" class="btn btn-primary btn-filter-trigger shadow-sm px-3" style="padding: 8px 15px;">
    ...
    <script>
    function abrirFiltrosTransferencias() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                document.getElementById('formFiltrosTransferencias').submit();
            } else if (result.dismiss === Swal.DismissReason.cancel) { window.location.href = 'ver_transferencias_ia.php'; }
        });
    }
    </script>
