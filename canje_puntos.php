...
<div class="container pb-5 mt-n4" style="position: relative; z-index: 20;">
    ...
    <?php if (!$cliente_seleccionado): ?>
        ...
        <div class="minimal-toolbar">
            <form id="formFiltrosCanje" method="GET" class="d-flex w-100 align-items-center gap-2 m-0">
                <div class="search-input-group">
                    <i class="bi bi-search"></i>
                    <input type="text" name="q" placeholder="Buscar cliente por nombre o DNI..." value="<?php echo htmlspecialchars($_GET['q'] ?? ''); ?>" autocomplete="off">
                </div>

                <input type="hidden" name="rango_puntos" id="hiddenRango" value="<?php echo htmlspecialchars($f_puntos); ?>">

                <button type="button" class="btn btn-light border btn-filter-trigger shadow-sm" onclick="abrirFiltrosCanje()">
        ...
        <script>
        function abrirFiltrosCanje() {
            let rango = document.getElementById('hiddenRango').value;
            Swal.fire({
                title: 'FILTRAR RANKING',
                html: `
                    <div style="text-align: left; display: flex; flex-direction: column; gap: 15px; margin-top: 10px;">
                        <div>
                            <label class="small fw-bold text-muted text-uppercase">Nivel de Puntos</label>
                            <select id="swal_rango" class="form-select fw-bold">
                                <option value="">Todos con puntos</option>
                                <option value="canjeables" ${rango === 'canjeables' ? 'selected' : ''}>Próximos a canjear (>= <?php echo $min_puntos_canje; ?> pts)</option>
                                <option value="vip" ${rango === 'vip' ? 'selected' : ''}>Clientes VIP (>2000 pts)</option>
                            </select>
                        </div>
                    </div>
                `,
                ...
                if (result.isConfirmed) {
                    document.getElementById('hiddenRango').value = document.getElementById('swal_rango').value;
                    document.getElementById('formFiltrosCanje').submit();
                } else if (result.dismiss === Swal.DismissReason.cancel) { window.location.href = 'canje_puntos.php'; }
            });
        }
        </script>
