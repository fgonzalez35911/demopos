...
<div class="container-fluid container-md mt-n4 px-2 px-md-3" style="position: relative; z-index: 20;">
    ...
    <div class="minimal-toolbar">
        <form id="formFiltrosGastos" method="GET" class="d-flex w-100 align-items-center gap-2 m-0">
            <div class="search-input-group">
                <i class="bi bi-search"></i>
                <input type="text" name="buscar" placeholder="Buscar detalle o Nº OP..." value="<?php echo htmlspecialchars($buscar); ?>" autocomplete="off">
            </div>
            ...
            <input type="hidden" name="categoria_filtro" id="hiddenCat" value="<?php echo htmlspecialchars($f_cat); ?>">
            <input type="hidden" name="id_usuario" id="hiddenUsuario" value="<?php echo htmlspecialchars($f_usu); ?>">

            <button type="button" class="btn btn-light border btn-filter-trigger shadow-sm" onclick="abrirFiltrosGastos()">
    ...
    <script>
    function abrirFiltrosGastos() {
        let usOptions = `<option value="">Todos</option>`;
        <?php 
        $usList = $conexion->query("SELECT id, usuario FROM usuarios ORDER BY usuario ASC")->fetchAll(PDO::FETCH_ASSOC);
        foreach($usList as $u): ?> 
        ...
        let st = document.getElementById('hiddenCat').value;
        Swal.fire({
            ...
            html: `
                <div style="text-align: left; display: flex; flex-direction: column; gap: 15px; margin-top: 10px;">
                    ...
                    <div>
                        ...
                        <select id="swal_cat" class="form-select fw-bold">
                            <option value="">Todas</option>
                            <option value="Proveedores" ${st === 'Proveedores' ? 'selected' : ''}>🚚 Proveedores</option>
                            <option value="Servicios" ${st === 'Servicios' ? 'selected' : ''}>💡 Servicios</option>
                            <option value="Alquiler" ${st === 'Alquiler' ? 'selected' : ''}>🏠 Alquiler</option>
                            <option value="Sueldos" ${st === 'Sueldos' ? 'selected' : ''}>👥 Sueldos</option>
                            <option value="Retiro" ${st === 'Retiro' ? 'selected' : ''}>💸 Retiro Ganancias</option>
                            <option value="Insumos" ${st === 'Insumos' ? 'selected' : ''}>🧻 Insumos</option>
                            <option value="Mermas" ${st === 'Mermas' ? 'selected' : ''}>📉 Mermas</option>
                            <option value="Otros" ${st === 'Otros' ? 'selected' : ''}>📦 Otros</option>
                        </select>
                    </div>
            ...
            if (result.isConfirmed) {
                ...
                document.getElementById('hiddenCat').value = document.getElementById('swal_cat').value;
                document.getElementById('hiddenUsuario').value = document.getElementById('swal_us').value;
                document.getElementById('formFiltrosGastos').submit();
            } else if (result.dismiss === Swal.DismissReason.cancel) { window.location.href = 'gastos.php'; }
        });
    }
    </script>
