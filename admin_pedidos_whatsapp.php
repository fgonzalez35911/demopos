...
<div class="container-fluid container-md mt-n4 px-2 px-md-3" style="position: relative; z-index: 20;">
    ...
    <div class="minimal-toolbar">
        <form id="formFiltrosPedidos" method="GET" class="d-flex w-100 align-items-center gap-2 m-0">
            <div class="search-input-group">
                <i class="bi bi-search"></i>
                <input type="text" name="buscar" placeholder="Buscar pedido o cliente..." value="<?php echo htmlspecialchars($buscar); ?>" autocomplete="off">
            </div>
            ...
            <input type="hidden" name="estado_filtro" id="hiddenEstado" value="<?php echo htmlspecialchars($f_est); ?>">

            <button type="button" class="btn btn-light border btn-filter-trigger shadow-sm" onclick="abrirFiltrosPedidos()">
    ...
    function abrirFiltrosPedidos() {
        ...
        Swal.fire({
                ...
                <div style="text-align: left; display: flex; flex-direction: column; gap: 15px; margin-top: 10px;">
                    ...
                    <div>
                        ...
                        <select id="swal_est" class="form-select fw-bold">
                            ...
                            <option value="pendiente" ${st === 'pendiente' ? 'selected' : ''}>⏳ Pendientes</option>
                            <option value="aprobado" ${st === 'aprobado' ? 'selected' : ''}>✅ Aprobados</option>
                            <option value="entregado" ${st === 'entregado' ? 'selected' : ''}>🛍️ Entregados</option>
                            <option value="rechazado" ${st === 'rechazado' ? 'selected' : ''}>❌ Rechazados</option>
                            <option value="no_retirado" ${st === 'no_retirado' ? 'selected' : ''}>🚫 No Retirados</option>
                        </select>
                    </div>
                </div>
            `,
            ...
            if (result.isConfirmed) {
                ...
                document.getElementById('hiddenEstado').value = document.getElementById('swal_est').value;
                document.getElementById('formFiltrosPedidos').submit();
            } else if (result.dismiss === Swal.DismissReason.cancel) { window.location.href = 'admin_pedidos_whatsapp.php'; }
        });
    }
    </script>
