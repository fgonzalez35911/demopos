...
<div class="container-fluid container-md pb-5 mt-n4 px-2 px-md-3" style="position: relative; z-index: 20;">
    ...
    <div class="minimal-toolbar">
        <form id="formFiltrosUsuarios" method="GET" class="d-flex flex-grow-1 align-items-center gap-2 m-0 w-100 w-md-auto">
            <div class="search-input-group">
                <i class="bi bi-search"></i>
                <input type="text" name="buscar" placeholder="Buscar nombre o @usuario..." value="<?php echo htmlspecialchars($buscar); ?>" autocomplete="off">
            </div>

            <input type="hidden" name="f_rol" id="hiddenRol" value="<?php echo htmlspecialchars($f_rol); ?>">
            <input type="hidden" name="f_estado" id="hiddenEstado" value="<?php echo htmlspecialchars($f_estado); ?>">

            <button type="button" class="btn btn-light border btn-filter-trigger shadow-sm" onclick="abrirFiltrosUsuarios()">
        ...
        <div class="d-flex gap-2">
            <a href="roles.php" class="btn btn-dark btn-filter-trigger shadow-sm">
                <i class="bi bi-shield-lock"></i> <span>ROLES</span>
            </a>
            <?php if($es_admin || in_array('config_gestionar_usuarios', $permisos)): ?>
                <button type="button" class="btn btn-success btn-filter-trigger shadow-sm" data-bs-toggle="modal" data-bs-target="#modalAlta">
                    <i class="bi bi-person-plus-fill"></i> <span>NUEVO</span>
                </button>
            <?php endif; ?>
        </div>
    </div>
    ...
    function abrirFiltrosUsuarios() {
        let rolOptions = `<option value="">Todos los roles</option>`;
        <?php foreach($roles_db as $r): ?> 
            rolOptions += `<option value="<?php echo $r['id']; ?>" ${document.getElementById('hiddenRol').value == '<?php echo $r['id']; ?>' ? 'selected' : ''}><?php echo addslashes(strtoupper($r['nombre'])); ?></option>`; 
        <?php endforeach; ?>
        ...
        Swal.fire({
            ...
            html: `
                <div style="text-align: left; display: flex; flex-direction: column; gap: 15px; margin-top: 10px;">
                    <div><label class="small fw-bold text-muted text-uppercase">Rango / Rol</label><select id="swal_rol" class="form-select fw-bold">${rolOptions}</select></div>
                    <div>
                        <label class="small fw-bold text-muted text-uppercase">Estado</label>
                        <select id="swal_estado" class="form-select fw-bold">
            ...
            if (result.isConfirmed) {
                document.getElementById('hiddenRol').value = document.getElementById('swal_rol').value;
                document.getElementById('hiddenEstado').value = document.getElementById('swal_estado').value;
                document.getElementById('formFiltrosUsuarios').submit();
            } else if (result.dismiss === Swal.DismissReason.cancel) { window.location.href = 'usuarios.php'; }
        });
    }
    </script>
