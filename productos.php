...
<style>
    ...
    @media (max-width: 768px) { 
        .filter-bar.sticky-desktop { top: 65px; margin-top: 15px; padding: 10px !important; }
...
<div class="container pb-5 mt-n4" style="position: relative; z-index: 20;">
    ...
    <div class="minimal-toolbar">
        <div class="search-input-group">
            ...
            <input type="text" id="buscador" placeholder="Buscar producto por nombre o código..." autocomplete="off">
        </div>

        <button type="button" class="btn btn-light border btn-filter-trigger shadow-sm" onclick="abrirFiltrosProductosSA2()">
        ...
        <input type="hidden" id="filtroCat" value="todos">
        <input type="hidden" id="filtroEstado" value="todos">
        <input type="hidden" id="ordenarPor" value="recientes">

        <div class="d-flex align-items-center bg-light border rounded-pill p-1 shadow-sm ms-2" style="height: 38px;">
            <div class="btn-group" role="group">
                <input type="radio" class="btn-check btn-check-grid" name="btnradioVista" id="btnGrid" autocomplete="off" checked onchange="cambiarVista(false)">
                <label class="btn btn-outline-primary border-0 rounded-pill btn-sm px-3 fw-bold m-0" for="btnGrid" title="Tarjetas"><i class="bi bi-grid-fill"></i></label>
                <input type="radio" class="btn-check btn-check-list" name="btnradioVista" id="btnList" autocomplete="off" onchange="cambiarVista(true)">
                <label class="btn btn-outline-primary border-0 rounded-pill btn-sm px-3 fw-bold m-0" for="btnList" title="Lista"><i class="bi bi-list-ul"></i></label>
            </div>
        </div>
    </div>

    <script>
    function abrirFiltrosProductosSA2() {
        let catOptions = `<option value="todos">📦 Todas las Categorías</option>`;
        <?php foreach($categorias as $c): ?>
            catOptions += `<option value="<?php echo $c['id']; ?>"><?php echo addslashes($c['nombre']); ?></option>`;
        <?php endforeach; ?>
        ...
        Swal.fire({
            ...
            html: `
                <div style="text-align: left; display: flex; flex-direction: column; gap: 15px; margin-top:10px;">
                    <div>
                        <label style="font-size: 0.75rem; font-weight: 800; color: #64748b; margin-bottom:5px;">CATEGORÍA</label>
                        <select id="swal_cat" class="form-select fw-bold" style="border-radius: 8px;">${catOptions}</select>
                    </div>
                    <div>
                        <label style="font-size: 0.75rem; font-weight: 800; color: #64748b; margin-bottom:5px;">ESTADO</label>
                        <select id="swal_est" class="form-select fw-bold" style="border-radius: 8px;">
                    ...
                    <div>
                        <label style="font-size: 0.75rem; font-weight: 800; color: #64748b; margin-bottom:5px;">ORDENAR POR</label>
                        <select id="swal_ord" class="form-select fw-bold" style="border-radius: 8px;">
            ...
            didOpen: () => {
                document.getElementById('swal_cat').value = document.getElementById('filtroCat').value;
                document.getElementById('swal_est').value = document.getElementById('filtroEstado').value;
                document.getElementById('swal_ord').value = document.getElementById('ordenarPor').value;
            },
            preConfirm: () => {
                document.getElementById('filtroCat').value = document.getElementById('swal_cat').value;
                document.getElementById('filtroEstado').value = document.getElementById('swal_est').value;
                document.getElementById('ordenarPor').value = document.getElementById('swal_ord').value;
                aplicarFiltros(); 
            }
        });
    }
...
const vistaKey = 'vista_productos_pref';
function cambiarVista(esLista) {
    localStorage.setItem(vistaKey, esLista ? 'lista' : 'grid');
    if (esLista) {
        ...
        document.getElementById('btnGrid').checked = true;
    }
}
document.addEventListener("DOMContentLoaded", function() {
    if (localStorage.getItem(vistaKey) === 'lista') {
        cambiarVista(true);
    }
});
