...
<div class="container-fluid container-md mt-n4 px-2 px-md-3" style="position: relative; z-index: 20;">
    ...
    <div class="minimal-toolbar">
        <form id="formFiltrosPremios" method="GET" class="d-flex w-100 align-items-center gap-2 m-0">
            <div class="search-input-group">
                <i class="bi bi-search"></i>
                <input type="text" name="buscar" placeholder="Buscar nombre o ID..." value="<?php echo htmlspecialchars($buscar); ?>" autocomplete="off">
            </div>

            <input type="hidden" name="desde_p" id="hiddenDesdeP" value="<?php echo htmlspecialchars($desde_p); ?>">
            <input type="hidden" name="hasta_p" id="hiddenHastaP" value="<?php echo htmlspecialchars($hasta_p); ?>">

            <button type="button" class="btn btn-light border btn-filter-trigger shadow-sm" onclick="abrirFiltrosPremios()">
    ...
    <script>
    function abrirFiltrosPremios() {
        Swal.fire({
            title: 'RANGO DE PUNTOS',
            html: `
                <div style="text-align: left; display: flex; flex-direction: column; gap: 15px; margin-top: 10px;">
                    <div><label class="small fw-bold text-muted text-uppercase">Puntos Mínimos</label><input type="number" id="swal_dp" class="form-control fw-bold" value="${document.getElementById('hiddenDesdeP').value}"></div>
                    <div><label class="small fw-bold text-muted text-uppercase">Puntos Máximos</label><input type="number" id="swal_hp" class="form-control fw-bold" value="${document.getElementById('hiddenHastaP').value}"></div>
                </div>
            `,
            ...
            if (result.isConfirmed) {
                document.getElementById('hiddenDesdeP').value = document.getElementById('swal_dp').value;
                document.getElementById('hiddenHastaP').value = document.getElementById('swal_hp').value;
                document.getElementById('formFiltrosPremios').submit();
            } else if (result.dismiss === Swal.DismissReason.cancel) { window.location.href = 'gestionar_premios.php'; }
        });
    }
    </script>
